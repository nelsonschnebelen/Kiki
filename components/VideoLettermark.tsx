...
interface VideoLettermarkProps {
  ...
  /**
   * "sequence": logo over the video letters, both exposed through refs.
   * "solid": logo only (hero in the reduced-motion fallback).
   * "posters": video letters with poster images, no video elements.
   */
  mode?: Mode;
 ...
 * same letter sits beneath it and reads as the crisp outline. The vector logo
 * sits above everything for the hero state; the scroll timeline crossfades one
 * into the other.
 */
  ...
  return (
    <div ref={rootRef} className="relative w-full" style={{ aspectRatio: `1 / ${g.letterAspect}` }}>
      ...
      {showSolid && (
        ...
        >
          {/* Vector logo: stays crisp from hero size to full width. */}
          <Image src={g.wordmark.svg} alt="" fill priority unoptimized className="object-contain" />
        </div>
      )}
    </div>
  );
