...
const dataBg = (d) => ({
  inputs: ["-stream_loop", "-1", "-i", `${W}/data-loop.mp4`],
  chain: `[0:v]trim=duration=${d},setpts=PTS-STARTPTS,${COVER},eq=brightness=-0.06:saturation=1.1`,
});

const CUT = [
  { id: "presents", dur: 3.6, ...black(3.6), over: [["presents", 0]], hit: [0.2] },
  ...
  { id: "d4", dur: 5.4, ...dataBg(5.4), over: [["d4", 0]], vo: [[17, 0.7]] },
  { id: "d5", dur: 6.2, ...dataBg(6.2), over: [["d5", 0]], vo: [[18, 0.5]] },
  { id: "d6", dur: 7.6, ...dataBg(7.6), over: [["d6", 0]], vo: [[19, 0.6]], fade: [0, 0.5] },
...
if (music) {
  ...
  ff(["-i", `${OUT}/kiki-trailer-vo-stem.wav`, "-i", music, "-filter_complex",
    `[1:a]atrim=duration=${TOTAL},afade=t=in:d=1.5,afade=t=out:st=${(TOTAL - 3).toFixed(2)}:d=3,volume=0.9[m];[0:a]asplit[vo][key];` +
