...
// feathered patch of the neighbouring sky before the crop is saved.
{
  ...
  const feather = Buffer.from(
    `<svg width="${patchW}" height="${patchH}"><defs><radialGradient id="g" cx="50%" cy="50%" r="50%">` +
      `<stop offset="68%" stop-color="#fff"/><stop offset="100%" stop-color="#fff" stop-opacity="0"/>` +
      `</radialGradient></defs><rect width="${patchW}" height="${patchH}" fill="url(#g)"/></svg>`,
  );
  const patch = await sharp(band)
    .extract({ left: 300, top: 270, width: patchW, height: patchH })
    .blur(4)
    .ensureAlpha()
  ...
  // sharp applies composite after resize, so patch first, then resize in a second pass.
  const patched = await sharp(band).composite([{ input: patch, left: 410, top: 265 }]).png().toBuffer();
  await sharp(patched)
...
// Footer. The mock bakes a wordmark into the crowd; cover it with a feathered
// blur of the crowd beside it so the live wordmark sits on a clean plate.
{
  const band = await sharp(FULL).extract({ left: 0, top: 1440, width: 941, height: 232 }).toBuffer();
  const patchW = 300;
  const patchH = 170;
  const feather = Buffer.from(
    `<svg width="${patchW}" height="${patchH}"><defs><radialGradient id="g" cx="50%" cy="50%" r="50%">` +
      `<stop offset="72%" stop-color="#fff"/><stop offset="100%" stop-color="#fff" stop-opacity="0"/>` +
      `</radialGradient></defs><rect width="${patchW}" height="${patchH}" fill="url(#g)"/></svg>`,
  );
  const patch = await sharp(band)
    .extract({ left: 20, top: 62, width: patchW, height: patchH })
    .blur(9)
    .ensureAlpha()
    .composite([{ input: feather, blend: "dest-in" }])
    .png()
    .toBuffer();
  const patched = await sharp(band).composite([{ input: patch, left: 320, top: 62 }]).png().toBuffer();
  await sharp(patched)
    .resize({ width: 1920, kernel: "lanczos3" })
    .jpeg({ quality: 84, mozjpeg: true })
    .toFile("public/images/nightlife.jpg");
  console.log("wrote public/images/nightlife.jpg");
}
